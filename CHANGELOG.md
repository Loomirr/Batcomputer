...
- Download updates in-app and choose when to restart. Existing settings and projects stay in place, with application-file backups available for recovery.
...
- Adds GitHub updates with full-ZIP downloads for this release.
...
- Shows the installed version below the logo, in the title bar and in Settings. Adds manual GitHub update checks, beta/stable selection, opt-in startup notifications, verified downloads, install-on-exit and retained recovery backups.
- Corrects the recognized Blender 100× root-unit representation before skeletal cooking, rebuilds inverse bind matrices, and requires the cooked root to match the native donor. This fixes oversized or distorted imported bodies and attachments. Earlier cached skinned imports require Reimport saved FBX.
...
- Excludes disposable acceptance/generated folders from SDK file discovery so stale extraction links cannot prevent the runtime catalog and registry-writer support files from being copied into development builds.
- Separates Home's Characters and Suits libraries and updates shared release/count/save wording. The command bar now uses responsive context cards, a readable character-aware Save action and tooltip details. The inspector restores a bounded width after leaving Home, including high-DPI layouts.
- Fixes normal character/suit packaging checking material dependencies before repairing saved texture cooks. Build preparation now validates/recooks the saved PNG recipes before stage repair or material resolution, preserves the saved design, and gives source-recovery instructions if recooking cannot succeed.
- Fixes character copying stopping on an outdated generated texture cook: new texture copies are cooked and certified before material dependencies are copied, without changing the original suit or weakening packaging validation. Character creation/identity, saved-design libraries, character-base selection and gameplay-donor menus now use shared workshop cards, live identity previews, inline validation, source filters and scrollable selection details.
- Adds a separate Characters workspace: independent roster/default-suit identity, the existing suit authoring categories, copying saved designs with isolated editable outputs, and character-owned child suits. Mod builds automatically include required defaults, group/progress assets, tags and additive roster configuration. New variants start unlocked; scripted story cutscene casting is not supported. No runtime DLL changes.
- Fixes native skeletal-part grafts failing when the target shell omits a default `AnimClass` property. Shared property updates now handle omitted defaults and reject conflicting field types. Part/cape/glider transactions no longer save their changes before replay succeeds, preserving rollback and newer-edit protection.
- Build Mod now waits for saved-suit restoration and automatically retries an incomplete authoring stage once from its exact saved recipe. Failures name the offending part/operation and explain recovery; stale builds and partial suits remain blocked.
- Fixes a cross-thread diagnostics exception when native belt/shoulder offsets or skinned meshes log from background staging. Messages now reach the UI asynchronously, including safe handling before log creation and during shutdown.
- Adds an experimental existing-rig FBX workshop for bodies and compatible skeletal parts: native rig export, external-weight validation, UE 5.6 cooking, per-slot materials, visual-component hiding, and a bone-pose preview. Source/cook revisions are project-owned and verified before staging both character roles. Facial rigs, cloth and skeletal equipment remain gated.
- Fixes custom hip/shoulder attachments retaining a head component tag. Adds native-derived, adjustable body clearance for belts and shoulder parts in gameplay and cutscenes, separately from accessory positioning, without stacking the same clearance on rebuild.
