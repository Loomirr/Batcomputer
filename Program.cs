...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        if (args.Length >= 1 && args[0].Equals("--redbrick-payload-proof", StringComparison.OrdinalIgnoreCase))
        {
            // --redbrick-payload-proof [outputRoot] [modId]
            // Creates a fresh, isolated payload/registry/trio/install tree. This
            // does not add a Red Brick UI feature yet; it decides whether native
            // RedBrickWorldSubsystem accepts independently authored payloads.
            var projectRoot = AppSettings.Current.EffectiveProjectRoot();
            var outputRoot = args.Length >= 2 && !string.IsNullOrWhiteSpace(args[1])
                ? args[1]
                : Path.Combine(AppSettings.GeneratedRootFor(projectRoot),
                    "RedBrickPayloadProof_" + DateTime.Now.ToString("yyyyMMdd_HHmmss"));
            var modId = args.Length >= 3 && !string.IsNullOrWhiteSpace(args[2])
                ? args[2]
                : "RedBrickPayloadProbe";
            var result = new RedBrickPayloadProbeService().CreateAsync(new RedBrickPayloadProbeService.Request
            {
                ExtractedContentRoot = AppSettings.Current.EffectiveExtractedContentRoot(),
                UsmapPath = AppSettings.Current.EffectiveUsmapPath() ?? "",
                OutputRoot = outputRoot,
                ModId = modId,
            }, Console.WriteLine).GetAwaiter().GetResult();
            Console.WriteLine($"status={result.Status}");
            Console.WriteLine($"target_package={result.TargetPackage}");
            Console.WriteLine($"asset_tag={result.AssetTag}");
            Console.WriteLine($"trio_base={result.TrioBasePath}");
            Console.WriteLine($"install_root={result.InstallRoot}");
            Console.WriteLine($"report={result.ReportPath}");
            if (!string.IsNullOrWhiteSpace(result.Error)) Console.Error.WriteLine("error=" + result.Error);
            return result.Status.Equals("created", StringComparison.OrdinalIgnoreCase) ? 0 : 1;
        }

        if (args.Length >= 1 && args[0].Equals("--redbrick-tint-proof", StringComparison.OrdinalIgnoreCase))
        {
            // --redbrick-tint-proof [outputRoot] [modId] [primary] [secondary] [tertiary]
            // Creates an unlocked, native-style custom Red Brick with tint data only.
            // It deliberately clears the donor VFX/overlay soft references; later UI
            // work can expose native effect presets as an opt-in authoring choice.
            var projectRoot = AppSettings.Current.EffectiveProjectRoot();
            var outputRoot = args.Length >= 2 && !string.IsNullOrWhiteSpace(args[1])
                ? args[1]
                : Path.Combine(AppSettings.GeneratedRootFor(projectRoot),
                    "RedBrickTintProof_" + DateTime.Now.ToString("yyyyMMdd_HHmmss"));
            var modId = args.Length >= 3 && !string.IsNullOrWhiteSpace(args[2])
                ? args[2]
                : "RedBrickTintProof";
            var result = new RedBrickTintProofService().CreateAsync(new RedBrickTintProofService.Request
            {
                ExtractedContentRoot = AppSettings.Current.EffectiveExtractedContentRoot(),
                UsmapPath = AppSettings.Current.EffectiveUsmapPath() ?? "",
                OutputRoot = outputRoot,
                ModId = modId,
                PrimaryColourRow = args.Length >= 4 ? args[3] : "BrightRed",
                SecondaryColourRow = args.Length >= 5 ? args[4] : "MediumBlue",
                TertiaryColourRow = args.Length >= 6 ? args[5] : "BrightYellow",
            }, Console.WriteLine).GetAwaiter().GetResult();
            Console.WriteLine($"status={result.Status}");
            Console.WriteLine($"effect_tag={result.EffectTag}");
            Console.WriteLine($"progress_tag={result.ProgressTag}");
            Console.WriteLine($"payload_package={result.PayloadPackage}");
            Console.WriteLine($"trio_base={result.TrioBasePath}");
            Console.WriteLine($"install_root={result.InstallRoot}");
            Console.WriteLine($"report={result.ReportPath}");
            if (!string.IsNullOrWhiteSpace(result.Error)) Console.Error.WriteLine("error=" + result.Error);
        ...
        if (args.Length >= 1 && args[0].Equals("--redbrick-early-init-proof", StringComparison.OrdinalIgnoreCase))
        {
            // --redbrick-early-init-proof [outputRoot] [modId] [primary] [secondary] [tertiary]
            // Diagnostic-only: overrides the original PROG_RedBricks package and
            // replaces one native default-unlocked tag with the custom proof tag.
            // This proves whether LiveData must be initialized from the base asset.
            var projectRoot = AppSettings.Current.EffectiveProjectRoot();
            var outputRoot = args.Length >= 2 && !string.IsNullOrWhiteSpace(args[1])
                ? args[1]
                : Path.Combine(AppSettings.GeneratedRootFor(projectRoot),
                    "RedBrickEarlyInitProof_" + DateTime.Now.ToString("yyyyMMdd_HHmmss"));
            var modId = args.Length >= 3 && !string.IsNullOrWhiteSpace(args[2])
                ? args[2]
                : "RedBrickEarlyInitProof";
            var result = new RedBrickTintProofService().CreateAsync(new RedBrickTintProofService.Request
            {
                ExtractedContentRoot = AppSettings.Current.EffectiveExtractedContentRoot(),
                UsmapPath = AppSettings.Current.EffectiveUsmapPath() ?? "",
                ...
                SecondaryColourRow = args.Length >= 5 ? args[4] : "MediumBlue",
                TertiaryColourRow = args.Length >= 6 ? args[5] : "BrightYellow",
                UseEarlyLiveEntryOverride = true,
            }, Console.WriteLine).GetAwaiter().GetResult();
            Console.WriteLine($"status={result.Status}");
            Console.WriteLine($"effect_tag={result.EffectTag}");
            Console.WriteLine($"progress_tag={result.ProgressTag}");
            Console.WriteLine($"progress_override={result.ProgressPackage}");
            Console.WriteLine($"trio_base={result.TrioBasePath}");
            Console.WriteLine($"install_root={result.InstallRoot}");
            Console.WriteLine($"report={result.ReportPath}");
            if (!string.IsNullOrWhiteSpace(result.Error)) Console.Error.WriteLine("error=" + result.Error);
        ...
        if (args.Length >= 1 && args[0].Equals("--redbrick-borrowed-ninja-proof", StringComparison.OrdinalIgnoreCase))
        {
            // --redbrick-borrowed-ninja-proof [outputRoot] [modId] [primary] [secondary] [tertiary]
            // Diagnostic-only: creates a distinct cosmetic tile and effect identity,
            // but borrows Ninja's native Red Brick Game Progress key. This does not
            // author a custom Game Progress definition or declare a custom progress
            // tag; it isolates whether the menu's lock state can come from the
            // existing native Ninja save entry.
            var projectRoot = AppSettings.Current.EffectiveProjectRoot();
            var outputRoot = args.Length >= 2 && !string.IsNullOrWhiteSpace(args[1])
                ? args[1]
                : Path.Combine(AppSettings.GeneratedRootFor(projectRoot),
                    "RedBrickBorrowedNinjaProof_" + DateTime.Now.ToString("yyyyMMdd_HHmmss"));
            var modId = args.Length >= 3 && !string.IsNullOrWhiteSpace(args[2])
                ? args[2]
                : "RedBrickBorrowedNinjaProof";
            var result = new RedBrickTintProofService().CreateAsync(new RedBrickTintProofService.Request
            {
                ExtractedContentRoot = AppSettings.Current.EffectiveExtractedContentRoot(),
                UsmapPath = AppSettings.Current.EffectiveUsmapPath() ?? "",
                OutputRoot = outputRoot,
                ModId = modId,
                DisplayName = "Borrowed Ninja Tint Test",
                PrimaryColourRow = args.Length >= 4 ? args[3] : "BrightRed",
                SecondaryColourRow = args.Length >= 5 ? args[4] : "MediumBlue",
                TertiaryColourRow = args.Length >= 6 ? args[5] : "BrightYellow",
                UseBorrowedNinjaProgressTag = true,
            }, Console.WriteLine).GetAwaiter().GetResult();
            Console.WriteLine($"status={result.Status}");
            Console.WriteLine($"asset_tag={result.AssetTag}");
            Console.WriteLine($"effect_tag={result.EffectTag}");
            Console.WriteLine($"borrowed_progress_tag={result.ProgressTag}");
            Console.WriteLine($"payload_package={result.PayloadPackage}");
            Console.WriteLine($"trio_base={result.TrioBasePath}");
            Console.WriteLine($"install_root={result.InstallRoot}");
            Console.WriteLine($"report={result.ReportPath}");
        ...
        if (args.Length >= 1 && args[0].Equals("--redbrick-unique-unlock-proof", StringComparison.OrdinalIgnoreCase))
        {
            // --redbrick-unique-unlock-proof [outputRoot] [modId] [primary] [secondary] [tertiary] [overrideDonorContentRoot]
            // Creates a completely mod-owned Red Brick: its own metadata, tint
            // payload, progress entry, and a one-entry native override collection
            // that marks only that entry Complete. The generated plugin adds a
            // temporary command-line shortcut purely to prove activation; release
            // activation will be owned by LOTDK Expanded, not user launch args.
            var projectRoot = AppSettings.Current.EffectiveProjectRoot();
            var outputRoot = args.Length >= 2 && !string.IsNullOrWhiteSpace(args[1])
                ? args[1]
                : Path.Combine(AppSettings.GeneratedRootFor(projectRoot),
                    "RedBrickUniqueUnlockProof_" + DateTime.Now.ToString("yyyyMMdd_HHmmss"));
            var modId = args.Length >= 3 && !string.IsNullOrWhiteSpace(args[2])
                ? args[2]
                : "RedBrickUniqueUnlockProof";
            var overrideRoot = args.Length >= 7 && !string.IsNullOrWhiteSpace(args[6])
                ? args[6]
                : Path.Combine(projectRoot, "_generated", "OverrideDonorExtract_20260803", "LEGOBatmanLotDK", "Content");
            var result = new RedBrickTintProofService().CreateAsync(new RedBrickTintProofService.Request
            {
                ExtractedContentRoot = AppSettings.Current.EffectiveExtractedContentRoot(),
                OverrideDonorContentRoot = overrideRoot,
                UsmapPath = AppSettings.Current.EffectiveUsmapPath() ?? "",
                OutputRoot = outputRoot,
                ModId = modId,
                DisplayName = "Unique Unlock Tint Test",
                PrimaryColourRow = args.Length >= 4 ? args[3] : "BrightRed",
                SecondaryColourRow = args.Length >= 5 ? args[4] : "MediumBlue",
                TertiaryColourRow = args.Length >= 6 ? args[5] : "BrightYellow",
                UseUniqueProgressUnlockOverride = true,
            }, Console.WriteLine).GetAwaiter().GetResult();
            Console.WriteLine($"status={result.Status}");
            Console.WriteLine($"progress_tag={result.ProgressTag}");
            Console.WriteLine($"progress_override={result.ProgressOverridePackage}");
            Console.WriteLine($"proof_command=-{result.ProgressOverrideCommand}");
            Console.WriteLine($"trio_base={result.TrioBasePath}");
            Console.WriteLine($"install_root={result.InstallRoot}");
            Console.WriteLine($"report={result.ReportPath}");
            if (!string.IsNullOrWhiteSpace(result.Error)) Console.Error.WriteLine("error=" + result.Error);
    ...
    /// <summary>Dumps a MaterialInstanceConstant's parent + scalar/vector/texture parameter values
    /// AND static-switch parameters (where Redbrick_ColourTint / RedBrickMode live). Used to diff a
    /// custom suit's material against a stock tinting material to find why Red Bricks don't apply.</summary>
