...
public sealed class ModReleaseValidationService
{
        ...
        string? installedContentPacksRoot)
    {
        ...
        // A combined mod may intentionally let one enabled suit reuse an icon generated by
        ...
        foreach (var input in enabled)
        {
            ...
            ValidateIcons(suit, releaseGeneratedTextures, result, suitId);
    ...
    internal static IReadOnlySet<string> ReleaseGeneratedTexturePackages(
        IEnumerable<SuitInput> inputs) =>
        inputs
            .Where(input => input.Entry.Enabled && input.Project is not null)
            .SelectMany(input => input.Project!.GeneratedTextures ?? Enumerable.Empty<GeneratedTextureEntry>())
    ...
    private static void ValidateIcons(
        NativeSuitProject suit,
        IReadOnlySet<string> releaseGeneratedTextures,
        Result result,
        string suitId)
    {
        ...
        })
        {
            ...
            else if (path.StartsWith("/Game/Mods/", StringComparison.OrdinalIgnoreCase) &&
                     !releaseGeneratedTextures.Contains(path))
            {
                result.AddError(
                    "icons",
                    $"The {slot} icon points at a mod texture that is not listed in this suit's generated textures. " +
                    "Select or reimport that icon before building; otherwise the UIMD would reference an asset that is not packaged.",
                    suitId);
            }
        }
    }
