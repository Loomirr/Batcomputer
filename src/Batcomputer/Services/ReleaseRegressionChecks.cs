...
internal static class ReleaseRegressionChecks
{
    public static int Run(TextWriter output)
    {
        ...
        foreach (var result in VehicleRegressionChecks.Run())
            Check(result.Passed, result.Description, failures, output);
        foreach (var result in VehicleContentRegressionChecks.Run())
            Check(result.Passed, result.Description, failures, output);
        foreach (var result in ModInstallTransactionRegressionChecks.Run())
            Check(result.Passed, result.Description, failures, output);
        foreach (var result in EditableModArchiveRegressionChecks.Run())
            Check(result.Passed, result.Description, failures, output);
        ...
        Check(ModelPreviewControl.IsCharacterExportDownload("blob:https://p1.batcomputer/123", "p1.batcomputer", "Character.glb") &&
              !ModelPreviewControl.IsCharacterExportDownload("https://example.org/model.glb", "p1.batcomputer", "Character.glb") &&
              !ModelPreviewControl.IsCharacterExportDownload("blob:https://p2.batcomputer/123", "p1.batcomputer", "Character.glb") &&
              !ModelPreviewControl.IsCharacterExportDownload("blob:https://p1.batcomputer/123", "p1.batcomputer", "run.exe"),
            "character export save dialog accepts only GLBs generated by the current local preview", failures, output);
        var twoHoleCape = faceMaterialRecipes.Single(recipe => recipe.Id == "cape.cloth.two-hole");
        Check(twoHoleCape.Outputs.Select(item => item.Role).OrderBy(role => role).SequenceEqual(
                new[] { "cutscene LOD0", "cutscene LOD1", "gameplay LOD0", "gameplay LOD1" }) &&
              twoHoleCape.Outputs.Select(item => item.DonorPackagePath).Distinct().Count() == 4 &&
              twoHoleCape.Outputs.All(item => item.DonorPackagePath.Contains("/TwoHole/Materials/") &&
                  item.DonorPackagePath.Contains("_CUT_") == item.Role.StartsWith("cutscene")) &&
              twoHoleCape.CompatibleMeshPackagePaths.Count == 2,
            "two-hole cape template preserves four distinct native context/LOD materials and restricts mesh family",
            failures, output);
            ...
            try
            {
                ...
                File.WriteAllText(
                    ...
                    System.Text.Json.JsonSerializer.Serialize(new NativeSuitPartIndex
                    {
                        ...
                        MappingsRevision = PartIndexService.CaptureMappingsRevision(AppSettings.Current.EffectiveUsmapPath())
                    }));
