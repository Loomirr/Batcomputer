...
local prefix = "[LOTDKJokerHarleyNormal] "
...
local function apply(startup)
    ...
    local note = startup and "F9 toggles this session." or "Return to title and reload to refresh the roster. Restart for a clean reset."
...
if type(IsKeyBindRegistered) == "function" and IsKeyBindRegistered(Key.F9) then
    print(prefix .. "F9 is already registered; shortcut not installed.\n")
    return
end
